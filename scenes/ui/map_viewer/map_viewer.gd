...
const _GRID_MATERIAL: Material = preload("uid://bo0owh3b3w2a3")
...
func _ready():
	...
	_map_select_option_button.item_selected.connect(_on_map_select_button_item_selected)
	_fog_of_war_toggle_button.toggled.connect(_on_fog_of_war_toggle_button_toggled)
	_render_mode_option_button.item_selected.connect(_on_render_mode_option_button_item_selected)
	_wrap_edges_toggle_button.toggled.connect(_on_wrap_edges_button_toggled)
	close_requested.connect(queue_free)
	
	var config_service: EditorConfigurationService = _injector.provide(EditorConfigurationService)
	_game_directory = config_service.get_game_install_directory()
...
func _change_map(map_file_path: String, script_file_path: String):
	var context = ParseContext.from_file(map_file_path)
	var map_file = MapFileModel.parse(context)
	...
	var offset = MapFileModel.MAP_SIZE * _MAP_CELL_SIZE
