...
const _MAP_SHADER: Shader = preload("uid://d3es71q8jp6sb")
...
func import_mesh(map_file: MapFileModel, script_file: ScriptFile, cell_size: float, \
		vertical_scale: float) -> Mesh:
	var vertex_rows = MapFileModel.MAP_SIZE + 1
	var vertex_columns = MapFileModel.MAP_SIZE + 1
	...
	for vertex_row in vertex_rows:
		for vertex_column in vertex_columns:
			...
			if vertex_row > 0 and !(vertex_row == 1 and vertex_column == 0):
				...
				if vertex_column == 0:
					cell_column = MapFileModel.MAP_SIZE - 1
					cell_row -= 1
...
func get_fog_of_war_mask(map_file: MapFileModel, discovered: Color = Color.WHITE,
		undiscovered: Color = Color.BLACK,
		format: Image.Format = Image.FORMAT_RGBA8) -> Image:
	var fog_of_war = Image.create(MapFileModel.MAP_SIZE, MapFileModel.MAP_SIZE, false,
			format)
	
	for row in MapFileModel.MAP_SIZE:
		for column in MapFileModel.MAP_SIZE:
...
## Returns the minimap.
func get_minimap(map_file: MapFileModel) -> Image:
	var minimap = Image.create(MapFileModel.MAP_SIZE, MapFileModel.MAP_SIZE, false,
			Image.FORMAT_RGBA8)
...
func _get_texture_index_map(map_file: MapFileModel) -> Texture2D:
	var texture_index_map = Image.create(MapFileModel.MAP_SIZE, MapFileModel.MAP_SIZE,
