...
func import(model: ModelFileModel) -> Array:
	...
	for frame in model.get_frames():
		...
		if frame_data is ModelFileModel.MapsFrameData:
			if textures == null:
	...
	for frame in model.get_frames():
		var frame_data = frame.get_data()
		if frame_data is ModelFileModel.MaterialFrameData:
			if materials == null:
	...
	for frame in model.get_frames():
		var frame_data = frame.get_data()
		if frame_data is ModelFileModel.DegrFrameData:
			if elements == null:
...
func _import_textures(maps: ModelFileModel.MapsFrameData) -> Array[Texture2D]:
...
func _import_materials(material_frame: ModelFileModel.MaterialFrameData, textures: Array[Texture2D]) -> Array[Material]:
	...
	for frame in material_frame.get_children():
		var frame_data = frame.get_data()
		if frame_data is ModelFileModel.MaterialAttributeFrameData:
...
func _import_degr_elements(degr: ModelFileModel.DegrFrameData, materials: Array[Material]) -> Array:
	assert(degr != null, 'Degr must not be null')
	
	var elements = []
	for frame in degr.get_children():
		var frame_data = frame.get_data()
		if frame_data is ModelFileModel.ElementFrameData:
...
func _import_element_objects(element: ModelFileModel.ElementFrameData, materials: Array[Material]) -> Array[Mesh]:
	assert(element != null, 'Element must not be null')
	
	var objects: Array[Mesh] = []
	for frame in element.get_children():
		var frame_data = frame.get_data()
		if frame_data is ModelFileModel.ObjectFrameData:
...
func _import_object_mesh(object: ModelFileModel.ObjectFrameData, materials: Array[Material]) -> Mesh:
	assert(object != null, 'Object must not be null')
	
	var vertices = PackedVector3Array()
	var normals = PackedVector3Array()
	var faces:Array[ModelFileModel.FaceArrayFrameData.Face] = []
	for frame in object.get_children():
		var frame_data = frame.get_data()
		if frame_data is ModelFileModel.VertexArrayFrameData:
			vertices.append_array(_import_vertices(frame_data))
		elif frame_data is ModelFileModel.NormalArrayFrameData:
			normals.append_array(_import_normals(frame_data))
		elif frame_data is ModelFileModel.FaceArrayFrameData:
...
func _import_vertices(vertices: ModelFileModel.VertexArrayFrameData) -> PackedVector3Array:
...
func _import_normals(normals: ModelFileModel.NormalArrayFrameData) -> PackedVector3Array:
...
func _import_faces(faces: ModelFileModel.FaceArrayFrameData) -> Array[ModelFileModel.FaceArrayFrameData.Face]:
...
func _build_mesh(vertices: PackedVector3Array, normals: PackedVector3Array, \
		faces: Array[ModelFileModel.FaceArrayFrameData.Face], materials: Array[Material]) -> Mesh:
	assert(vertices != null, 'Vertices must not be null')
	assert(normals != null, 'Normals must not be null')
	assert(faces != null, 'Faces must not be null')
