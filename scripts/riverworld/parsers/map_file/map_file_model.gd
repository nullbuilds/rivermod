...
static func parse(context: ParseContext) -> MapFileModel:
	var file_size: int = context.get_remaining_bytes()
	...
	while context.has_remaining_bytes():
		cells.append(MapCell.parse(context))
	
	var map: MapFileModel = MapFileModel.new()
...
class MapCell:
	...
	var _unknown_field: int = 0
	var _flags: int = 0
	
	## Attempts to deserialize a map cell from the provided context.
	static func parse(context: ParseContext) -> MapCell:
